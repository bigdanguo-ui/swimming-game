...
static glm::vec2 computePlanarUV(const glm::vec3& position, const glm::vec3& normal)
{
...
std::vector<glm::vec2> TriMesh::getTexCoords()
{
	return texcoords;
}
...
void TriMesh::storeFacesPoints() {
	...
	for (int i = 0; i < faces.size(); i++)
	{
		...
		if (face_normals.size() == faces.size()) {
			faceNormal = face_normals[i];
		}
		// 坐标
		glm::vec3 p0 = vertex_positions[faces[i].x];
		glm::vec3 p1 = vertex_positions[faces[i].y];
		glm::vec3 p2 = vertex_positions[faces[i].z];
		points.push_back(p0);
		points.push_back(p1);
		points.push_back(p2);
		// 颜色
		...
		// 纹理坐标
		texcoords.push_back(computePlanarUV(p0, faceNormal));
		texcoords.push_back(computePlanarUV(p1, faceNormal));
		texcoords.push_back(computePlanarUV(p2, faceNormal));
		// 法向量
		if (vertex_normals.size() != 0) {
